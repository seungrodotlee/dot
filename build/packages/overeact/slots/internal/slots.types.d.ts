...
type ToCamelCase<T> = T extends `${infer First}${infer Rest}` ? `${Lowercase<First>}${Rest}` : T;
export type ToPascalCase<T> = T extends `${infer First}${infer Rest}` ? `${Uppercase<First>}${Rest}` : T;
